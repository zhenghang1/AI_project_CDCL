...
class SAT:
    ...
    def __init__(self, init_decider:str, restarter:str, sentence:list[list[int]], num_vars:int):
        '''
        ...
        self._stats = Statistics(num_vars)
        ...
        self._c2l_watch, self._l2c_watch = self.init_watch()
        ...
        self._decider = Decider(init_decider, self._sentence, self._stats._num_vars)
    ...
    def init_watch(self):
        ...
        for lit in range(-self._stats._num_vars, self._stats._num_vars + 1):
            l2c_watch[lit] = []

        for clause_idx, clause in enumerate(self._sentence):  # for each clause watch first two literals
    ...
    def cdcl_run(self):
        ...
        while len(self._assignment) < self._stats._num_vars:
            # Make a decision.
            assigned_lit = self._decider.decide_lit()
